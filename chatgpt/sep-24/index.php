        ...
        <?php $index = 1; foreach ($data as $row): ?>
            <tr class='table-data-cell'>
                ...
                <?php
                    ...
                    // Log the imageOrder array
                    file_put_contents('image_order_log.txt', "Image Order before usort():\n" . print_r($imageOrder, true) . "\n", FILE_USE_INCLUDE_PATH);
                    
                    // Log the imgPaths array
                    file_put_contents('img_paths_log.txt', "Image Paths before usort():\n" . print_r($imgPaths, true) . "\n", FILE_USE_INCLUDE_PATH);
                    
                    file_put_contents('my_log.txt', "Before sorting: " . json_encode($imgPaths) . "\n", FILE_USE_INCLUDE_PATH);
                    // Reorder $imgPaths based on $imageOrder
                    ...
                    // Log the comparisons
                    file_put_contents('my_log.txt', "Comparing: $aName ($aPosition) vs $bName ($bPosition)\n", FILE_APPEND);
            
                        if ($aPosition === false && $bPosition === false) return 0;  // Neither image is in the image_order table
                        if ($aPosition === false) return 1;   // Only $b is in the image_order table
                        if ($bPosition === false) return -1;  // Only $a is in the image_order table
            
                        return $aPosition - $bPosition;  // Compare positions in the image_order table
                    });
                    file_put_contents('my_log.txt', "After sorting: " . json_encode($imgPaths) . "\n", FILE_APPEND);
